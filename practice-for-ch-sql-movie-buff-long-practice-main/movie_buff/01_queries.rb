...
def biggest_cast
  ...
  # actors).
  Movie.joins(:castings).group(:id).order("COUNT(movies.id) DESC").limit(3).select("movies.id, movies.title")
end
...
def directed_by_one_of(them)
  ...
  # Actor.select("movies.id, movies.title").join(:directed_movies).where('actors.id IN (?)', them)
end
