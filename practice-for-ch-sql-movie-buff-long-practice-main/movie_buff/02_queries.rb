...
def bad_years
  ...
  Movie.where('score < 8 AND yr NOT IN (
    SELECT
      yr
    FROM
      movies
...
def cast_list(title)
  ...
  Actor.select('actors.id, actors.name').joins(:movies).where('movies.title = (?)', title).order("ord ASC")
end
...
def vanity_projects
  ...
  Movie.select('movies.id, title, actors.name').joins(:actors).where('director_id = actor_id AND castings.ord = 1')
end

def most_supportive
  # Find the two actors with the largest number of non-starring roles.
  # Show each actor's id, name, and number of supporting roles.
  Actor.select('actors.id, actors.name, COUNT(actors.id) AS num_roles')
  .join(:castings)
